...
export function DashboardSidebar({
  ...
  return (
    <Sidebar className="bg-[#0c172b] text-white border-r-0 shadow-[4px_0_32px_rgba(12,23,43,0.18)]" collapsible="icon">
      {/* Brand Header */}
      <SidebarHeader className="flex items-center gap-3 p-4 border-b border-white/[0.07] min-h-[76px] overflow-hidden">
        <div
          className="w-11 h-11 rounded-2xl shrink-0 flex items-center justify-center shadow-[0_12px_32px_rgba(249,115,22,0.28)]"
          style={{
            background: "radial-gradient(circle at 35% 25%, rgba(255,255,255,.95) 0 10%, transparent 11%), linear-gradient(135deg, #f97316, #c2410c)",
          }}
        >
          <span className="font-extrabold text-base text-white font-display">M</span>
        </div>
              ...
              <SidebarMenuItem key={item.label}>
                ...
                >
                  <Link href={item.href} onClick={onCloseSidebar}>
                    ...
                    <span className="text-[.86rem] font-[760] tracking-[-0.015em]">{item.label}</span>
                  </Link>
                </SidebarMenuButton>
              </SidebarMenuItem>
            );
      ...
      <SidebarFooter className="p-3 border-t border-white/[0.07] gap-1">
        <SidebarMenu className="gap-1">
          <SidebarMenuItem>
            <SidebarMenuButton
              asChild
              tooltip="Pengaturan"
              className="text-white/40 hover:bg-white/10 hover:text-white/90 rounded-2xl min-h-[42px] transition-colors cursor-pointer"
            >
              <Link href="/dashboard/umkm/pengaturan" onClick={onCloseSidebar}>
                <Settings className="size-5 shrink-0" />
                <span className="text-[.82rem] font-[720]">Pengaturan</span>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
          <SidebarMenuItem>
            ...
            >
              <Link href="#">
                ...
                <span className="text-[.82rem] font-[720]">Bantuan</span>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
          <SidebarMenuItem>
            <SidebarMenuButton
              asChild
              tooltip="Keluar"
              className="text-red-400/70 hover:bg-white/5 hover:text-red-300 rounded-2xl min-h-[42px] transition-colors cursor-pointer"
            >
              <Link href="/">
                <LogOut className="size-5 shrink-0" />
                <span className="text-[.82rem] font-[720]">Keluar</span>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
        </SidebarMenu>
        ...
        {/* User identity strip */}
        <div className="flex items-center gap-3 mt-2 px-2 py-3 rounded-2xl overflow-hidden group-data-[collapsible=icon]:justify-center">
          <div
            className="w-11 h-11 shrink-0 rounded-2xl shadow-[0_8px_20px_rgba(249,115,22,0.16)]"
