  ...
  try {
    ...
    if (!authUser) {
      return json(
        ...
        {
          success: false,
          error: "Kode OTP salah atau kedaluwarsa. Minta kode baru.",
        },
        401,
      );
    }
    ...
    return json(
      res,
      { success: false, error: "Gagal mengatur ulang password. Coba lagi." },
      500,
    );
  }
};
...
function validateInput({ email, otpCode, password }) {
  ...
  if (password.length > PASSWORD_MAX)
    return `Password maksimal ${PASSWORD_MAX} karakter.`;
  return "";
}
