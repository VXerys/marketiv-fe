 ...
 * satu mutasi hilang. Untuk `campaigns.totalClaims` itu berarti kuota tidak
 * pernah dikembalikan dengan benar saat klaim kedaluwarsa. Event database bisa
 * terkirim ulang, jadi ini bukan kasus teoretis.
 *
 ...
 * major SDK di Function jalur uang menambah risiko yang tidak sebanding —
 ...
 *
 * ⚠️ SALINAN. Berkas ini identik di:
 *   - calculate-campaign-reward/src/atomic.js
 *   - create-escrow/src/atomic.js
 *   - release-escrow/src/atomic.js
 *   - mature-pending-balance/src/atomic.js
 *   - campaign-claimed/src/atomic.js
 *   - expire-stale-claims/src/atomic.js  ← berkas ini
 * Deploy root Appwrite adalah `functions/<id>/`, jadi modul di luar folder itu
 * TIDAK ikut ter-bundle — direktori bersama akan menghasilkan Function yang lolos
 * build lalu gagal saat runtime. Ubah satu, ubah semuanya.
 *
